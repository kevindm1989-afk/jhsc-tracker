...
import { Upload, FileSpreadsheet, CheckCircle2, AlertTriangle, XCircle, Users, MapPin, SkipForward } from "lucide-react";
...
export default function ImportMinutesPage() {
  ...
  const handleFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      const resp = await fetch(`${BASE}/api/import/minutes?preview=true`, {
        method: "POST",
        body: formData,
      });
      if (!resp.ok) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${BASE}/api/import/minutes`, {
        method: "POST",
        body: formData,
      });
      if (!resp.ok) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">Import Minutes</h1>
        <p className="text-muted-foreground mt-1 text-sm">Upload your JHSC meeting minutes workbook to sync action items and hazard findings.</p>
      </div>
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-white rounded-md border border-green-200 p-3 text-center">
                <p className="text-2xl font-bold text-green-700">{result.imported.actionItems}</p>
                <p className="text-xs text-muted-foreground mt-1">Action Items Imported</p>
              </div>
              <div className="bg-white rounded-md border border-green-200 p-3 text-center">
                <p className="text-2xl font-bold text-green-700">{result.imported.hazardFindings}</p>
                <p className="text-xs text-muted-foreground mt-1">Hazard Findings Imported</p>
              </div>
              <div className="bg-white rounded-md border border-slate-200 p-3 text-center">
                <p className="text-2xl font-bold text-muted-foreground">{result.skipped.actionItems}</p>
                <p className="text-xs text-muted-foreground mt-1">Action Items Skipped</p>
              </div>
              <div className="bg-white rounded-md border border-slate-200 p-3 text-center">
                <p className="text-2xl font-bold text-muted-foreground">{result.skipped.hazardFindings}</p>
                <p className="text-xs text-muted-foreground mt-1">Hazard Findings Skipped</p>
              </div>
            </div>
      ...
      {/* Upload area (hide after success) */}
      {!result && (
        <div
        ...
        >
          ...
          {file ? (
            ...
            <div>
              <p className="font-medium text-foreground">Drop your minutes file here</p>
              <p className="text-sm text-muted-foreground mt-1">or click to browse — .xlsx, .xlsm supported</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <Card className="border-sidebar-border shadow-sm">
            ...
            <CardContent className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
              <div>
                <p className="text-xs font-bold uppercase text-muted-foreground mb-1">Meeting Date</p>
                <p className="font-medium">{preview.meetingDate || "—"}</p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-muted-foreground mb-1">Facility</p>
                <p className="font-medium">{preview.facility || "—"}</p>
              </div>
              <div>
                <p className="text-xs font-bold uppercase text-muted-foreground mb-1">Quorum Met</p>
                <p className="font-medium capitalize">{preview.quorumMet || "—"}</p>
              </div>
            </CardContent>
          </Card>
              ...
              <CardHeader className="pb-3 border-b bg-destructive/5">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-bold uppercase tracking-wide flex items-center gap-2 text-destructive">
                    ...
                    Hazard Findings / Recommendations to Import ({preview.hazardFindings.length})
                  </CardTitle>
                </div>
                <CardDescription className="text-xs flex items-center gap-1 mt-1">
                  <SkipForward className="w-3 h-3" />
                ...
                <div className="divide-y max-h-60 overflow-y-auto">
                  {preview.hazardFindings.map((hf, i) => (
                    <div key={i} className="px-4 py-2.5 flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-3 text-sm hover:bg-muted/30">
                      ...
                      <span className="font-medium text-foreground leading-snug min-w-0 line-clamp-2" title={hf.hazardDescription}>{hf.hazardDescription}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            >
              ...
              {isImporting
                ? "Importing..."
                : `Import ${preview.actionItems.length} Action Items + ${preview.hazardFindings.length} Hazard Findings`}
            </Button>
          ...
          <CardContent className="p-6 text-center text-sm text-muted-foreground space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs mt-3">
              <div className="space-y-1">
                <p className="font-bold text-foreground">1. Upload</p>
                <p>Drop your JHSC meeting minutes .xlsm or .xlsx file above.</p>
              </div>
              <div className="space-y-1">
                <p className="font-bold text-foreground">2. Preview</p>
                <p>Review the action items and hazard findings that will be imported.</p>
              </div>
              <div className="space-y-1">
                <p className="font-bold text-foreground">3. Confirm</p>
                <p>Duplicates are skipped automatically — safe to re-import any time.</p>
              </div>
            </div>
          </CardContent>
        </Card>
